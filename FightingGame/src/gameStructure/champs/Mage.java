...
public class Mage extends Champion {
	...
	public Mage(GameBaseApp app, String[] c) {
		...
		/*
		 * stand = new Animation(app, standingImg, 100); walk = new
		 * Animation(app, walkingImg, 800); death = new Death(app, attackImg,
		 * 500); basicAttack = new Attack(app, attackImg, 600);
		 * 
		 * setAnimation(walk);
		 */
		// ************************************
		...
		getStats().getHp().initAmount(500);
		getStats().setArmor(30);
		getStats().setMagicResist(30);

		getStats().getMovementSpeed().setBrutoBaseAmount(120);
		getStats().getRadius().setBrutoBaseAmount(15);
		System.out.println("Stats.setSight()" + 100);
		getStats().getSight().setBrutoBaseAmount(100);

		aggroRange = (byte) (getStats().getRadius().getTotalAmount() + 50);
		/*
		 * basicAttack.range = 9; basicAttack.damage = 40; basicAttack.cooldown
		 * = 2000; basicAttack.setCastTime(500);
		 * 
		 * descr = " ";
		 */
		// ************************************
	...
	public void doAttack(Attack a) {
		player.app.getUpdater().send("<hit " + basicAttack.getTarget().getNumber() + " " + a.getDamage() + " ");
	}
