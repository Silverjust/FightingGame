...
public class Destructor extends Unit implements Shooter, Buffing {
	...
	public void calculateDamage(Attack a) {
		...
		for (GameObject e : GameApplet.GameBaseApp.gameObjects) {
			if (e != null & e.isEnemyTo(this) && e.isInRange(target.getX(), target.getY(), e.getStats().getRadius() + splashrange)
	...
	static class RuglingSpawn extends Ability {
		...
		public void updateAbility(GameObject e, boolean isServer) {
			if (target != null && isEvent()) {
				if (isServer) {
					GameBaseApp.getUpdater().sendDirect(SPAWN + " Orb " + e.player.getUser().getIp() + " " + e.getX() + " " + (e.getY() + e.getStats().getRadius() + 8)
